...
def lisp_get_port_on_command_line():
    ...
    for p in ["443", "-8082", "8082"]: #[added code]
...
def lisp_bottle_ipv4_process(bottle_port):
    ...
    #
    if (bottle_port < 0):
        bottle.run(host="0.0.0.0", port=-bottle_port)# [added code] 0.0.0.0 -> 10.10.10.80
        return
    #endif
...
bottle_port = int(sys.argv[1]) if (len(sys.argv) > 1) else 8082 # 8080 [added code]
