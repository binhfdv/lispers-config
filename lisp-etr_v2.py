...
LISP_MAP_REGISTER_INTERVAL = 30 # In units of seconds
...
# [added code]
import json
eid_list = ['eid1', 'eid3'] # service ids

def read_js(fp, eid_list):
  with open(fp, 'r') as json_file:
    js_data = json.load(json_file)
    # find the latest date
    dates = [i['date'] for i in js_data]
    l = sorted(dates, key = lambda i: i, reverse = True)[0]
    # find eids
    resource = []
    for i in js_data[::-1]:
      if i['date'] == l:
...
def lisp_build_map_register_records(quiet, db, eid, group, ttl):
    ...
    for iid in [eid.instance_id] + eid.iid_list:
        ...
        eid_record.computing_resource = read_js('./db_resources.json', eid_list) #[added code]
...
def lisp_build_map_register(lisp_sockets, ttl, eid_only, ms_only, refresh):
    ...
    #
    # Create data structure instances to build Map-Register message.
    #
    ...
    for ms in list(lisp.lisp_map_servers_list.values()):
        ...
        for msl in ms_list[ms_dns_name]:
            ...
            map_register.computing_resource = read_js('./db_resources.json', eid_list) #[added code]
...
#       if (source != None): continue
    ...
    for source, group, iid, ms_dns_name, rle_addr, joinleave in entries:
        ...
        eid_record = lisp.lisp_eid_record()
        eid_record.computing_resource = 101 # [added code]
    ...
    for ms in list(lisp.lisp_map_servers_list.values()):
        ...
        map_register.xtr_id = ms.xtr_id
        map_register.site_id = ms.site_id
        map_register.computing_resource = 102 #[added code]
        map_register.encrypt_bit = (ms.ekey != None)
        packet = map_register.encode()
        map_register.print_map_register()
